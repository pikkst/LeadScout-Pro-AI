...
async function startServer() {
  ...
  if (!isProduction) {
    try {
      ...
      console.log("[Backend Server] Vite development middleware loaded successfully.");
    } catch (err) {
      console.warn("[Backend Server] Failed to load Vite development middleware (likely due to missing devDependencies in production). Falling back to static production mode.", err);
      isProduction = true;
    }
  }
  ...
  if (isProduction) {
    ...
    console.log("[Backend Server] Serving production static files from: " + distPath);
  }

  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[Backend Server] Server bound to 0.0.0.0:${PORT} in ${isProduction ? 'production' : 'development'} mode.`);
  });
}
